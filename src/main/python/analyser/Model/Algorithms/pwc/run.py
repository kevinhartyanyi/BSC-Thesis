...
assert (
    int(str("").join(torch.__version__.split(".")[0:2])) >= 10
)  # requires at least pytorch version 1.0.0
...
def Backward(tensorInput, tensorFlow):
    ...
    tensorOutput = torch.nn.functional.grid_sample(
        input=tensorInput,
        grid=(Backward_tensorGrid[str(tensorFlow.size())] + tensorFlow).permute(
            0, 2, 3, 1
        ),
        mode="bilinear",
        padding_mode="zeros",
    ...
    def __init__(self):
        ...
        class Decoder(torch.nn.Module):
            def __init__(self, intLevel):
                ...
                if intLevel < 6:
                    self.dblBackward = [None, None, None, 5.0, 2.5, 1.25, 0.625, None][
                        intLevel + 1
                    ]
                ...
                if objectPrevious is None:
                    ...
                    tensorVolume = torch.nn.functional.leaky_relu(
                        input=correlation.FunctionCorrelation(
                            tensorFirst=tensorFirst, tensorSecond=tensorSecond
                        ),
                        negative_slope=0.1,
                        inplace=False,
                    )
...
def run():
    ...
    tensorFirst = torch.FloatTensor(
        numpy.array(PIL.Image.open(arguments_strFirst))[:, :, ::-1]
        .transpose(2, 0, 1)
        .astype(numpy.float32)
        * (1.0 / 255.0)
    )
    tensorSecond = torch.FloatTensor(
        numpy.array(PIL.Image.open(arguments_strSecond))[:, :, ::-1]
        .transpose(2, 0, 1)
        .astype(numpy.float32)
        * (1.0 / 255.0)
    )
    ...
    numpy.array([tensorOutput.size(2), tensorOutput.size(1)], numpy.int32).tofile(
        objectOutput
    )
    tensorOutput = tensorOutput.detach()  #! Change
    numpy.array(tensorOutput.numpy().transpose(1, 2, 0), numpy.float32).tofile(
        objectOutput
    )
