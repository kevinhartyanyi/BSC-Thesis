...
def calculateVelocityAndOrientationVectors(
    labels, shifted_labels, avg_flow, avg_fst_depth, avg_shifted_depth
):
    """
    ...
    h_angle, v_angle = calcAngleOfView(
        width_to_focal[labels.shape[1]], avg_flow[:, :, 1], avg_flow[:, :, 0]
    )
...
def calculateVelocityAndOrientationVectorsVectorised(
    of_mask, next_position, prev_position, flow, fst_depth, snd_depth
):
...
def calcBidiErrormap(flowAB, flowBA, tau=1.0, consistent_flow=False):
    ...
    x_mat = (np.expand_dims(range(w), 0) * np.ones((h, 1), dtype=np.int32)).astype(
        np.int32
    )
    y_mat = (np.ones((1, w), dtype=np.int32) * np.expand_dims(range(h), 1)).astype(
        np.int32
    )
...
def drawVelocityVectors(
    ...
    if not relative_disp:
        x_mat = (
            np.expand_dims(range(width), 0) * np.ones((height, 1), dtype=np.int32)
        ).astype(np.int32)
        y_mat = (
            np.ones((1, width), dtype=np.int32) * np.expand_dims(range(height), 1)
        ).astype(np.int32)
        disparities[..., 0] -= y_mat
        disparities[..., 1] -= x_mat
...
def readDepth(depth_fn, width, height):
    ...
    np.multiply(
        ...
        np.divide(baseline, np.multiply(width, disparity)),
        out=disparity,
    )
...
def listDirectory(dir_name, extension=None):
    ...
    fns = [
        ...
        if is_valid(os.path.join(dir_name, fn))
    ]
    fns = natsorted(fns)
    return fns
...
def reduceSort(vector, low=0.1, high=0.9, skip=None):
    ...
    mask_vector = np.logical_and(
        vector >= v_unique[int(len(v_unique) * low)],
        vector <= v_unique[int(len(v_unique) * high) - 1],
    )
    return mask_vector


def vectorSpeedOFSimple(vectors, high=1, low=0, optimal_of=True):
    ...
    x = vectors[:, :, 0]
    y = vectors[:, :, 1]
    mask_uni = reduceSort(y, low=low, high=high)
    print(low, high)
    if optimal_of:
        only_good_of = x != 0
    ...
    if (
        np.mean(ofhor_left) * np.mean(ofhor_right) > 1
    ):  # left and right horizontal OF have the same sign, i.e. turning
...
def errorComparisonSpeedVectors(speed_est, speed_gt, csv=None, visualize=True):
    ...
    """
    rmse = np.sqrt(
        np.mean(np.square(speed_est - speed_gt), 0)
    ...
    for frame in range(0, len(speed_gt)):
        print(
            ("{:.2f}" + spaces + "{:.2f}" + spaces + "{:.2f}").format(
                speed_gt[frame], speed_est[frame], speed_gt[frame] - (speed_est[frame])
            )
        )
        csv_list.append(
            {
                "Truth": speed_gt[frame],
                "Est": speed_est[frame],
                "Error": speed_gt[frame] - (speed_est[frame]),
            }
        )
