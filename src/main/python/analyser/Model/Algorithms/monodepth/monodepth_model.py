...
from Model.Algorithms.monodepth.bilinear_sampler import *
...
class MonodepthModel(object):
    ...
    def get_disparity_smoothness(self, disp, pyramid):
        ...
        weights_x = [
            tf.exp(-tf.reduce_mean(tf.abs(g), 3, keep_dims=True))
    ...
    def build_outputs(self):
        ...
        with tf.variable_scope("images"):
            self.left_est = [
                self.generate_image_left(self.right_pyramid[i], self.disp_left_est[i])
                for i in range(4)
            ]
            self.right_est = [
                self.generate_image_right(self.left_pyramid[i], self.disp_right_est[i])
                for i in range(4)
            ]
        ...
        with tf.variable_scope("left-right"):
            self.right_to_left_disp = [
                self.generate_image_left(self.disp_right_est[i], self.disp_left_est[i])
                for i in range(4)
            ]
            self.left_to_right_disp = [
                self.generate_image_right(self.disp_left_est[i], self.disp_right_est[i])
                for i in range(4)
            ]
    ...
    def build_losses(self):
        with tf.variable_scope("losses", reuse=self.reuse_variables):
            ...
            # L1
            self.l1_left = [
                tf.abs(self.left_est[i] - self.left_pyramid[i]) for i in range(4)
            ]
            self.l1_reconstruction_loss_left = [tf.reduce_mean(l) for l in self.l1_left]
            self.l1_right = [
                tf.abs(self.right_est[i] - self.right_pyramid[i]) for i in range(4)
            ]
            self.l1_reconstruction_loss_right = [
                tf.reduce_mean(l) for l in self.l1_right
            ]

            # SSIM
            self.ssim_left = [
                self.SSIM(self.left_est[i], self.left_pyramid[i]) for i in range(4)
            ]
            self.ssim_loss_left = [tf.reduce_mean(s) for s in self.ssim_left]
            self.ssim_right = [
                self.SSIM(self.right_est[i], self.right_pyramid[i]) for i in range(4)
            ]
            ...
            self.lr_left_loss = [
                tf.reduce_mean(
                    tf.abs(self.right_to_left_disp[i] - self.disp_left_est[i])
                )
                for i in range(4)
            ]
            self.lr_right_loss = [
                tf.reduce_mean(
                    tf.abs(self.left_to_right_disp[i] - self.disp_right_est[i])
                )
                for i in range(4)
            ]
    ...
    def build_summaries(self):
        ...
        with tf.device("/cpu:0"):
            ...
            if self.params.full_summary:
                tf.summary.image(
                    "left", self.left, max_outputs=4, collections=self.model_collection
                )
                tf.summary.image(
                    "right",
                    self.right,
