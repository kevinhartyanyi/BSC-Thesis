...
class VelocityCalculator(object):
    ...
    def calculate_velocity_and_orientation(self):
        if self.super_pixel_method != "":
            ...
            fst_img = cv2.imread(
                self.fst_img_fn, cv2.IMREAD_COLOR
            )  # height x width x channels
            snd_img = cv2.imread(
                self.snd_img_fn, cv2.IMREAD_COLOR
            )  # height x width x channels
            ...
            avg_flow[:, :, 0] = self.average(
                flow[:, :, 0], labels, labels_unique, label_masks
            )
            avg_flow[:, :, 1] = self.average(
                flow[:, :, 1], labels, labels_unique, label_masks
            )
            ...
            avg_shifted_depth = self.average(
                snd_depth, shifted_labels, labels_unique, label_masks
            )

            # Calculate the velocity and the orientation
            velocity = utils.calculateVelocityAndOrientationVectors(
                labels, shifted_labels, avg_flow, avg_fst_depth, avg_shifted_depth
            )

            speed, speed_mask = utils.vectorSpeedOFSimple(
                velocity, low=self.low, high=self.high
            )  # Speed calculation
            ...
            utils.saveAsImage(
                os.path.join(base_fn, MASK_DIR, img_num + "_speed_masked.png"),
                speed_mask * 50,
                min_val=0,
                max_val=utils.max_depth,
            )
            np.save(os.path.join(base_fn, NP_DIR, img_num + "_velocity.npy"), velocity)

            if self.create_draw:
                back_flow = self.read_flow(self.back_flow)
                of_mask, next_position, prev_position = utils.calcBidiErrormap(
                    flow, back_flow, tau=0.8
                )

                incons_img = np.tile(
                    255 * np.ones(of_mask[..., None].shape), (1, 1, 3)
                ).astype(np.uint8)
                incons_img = utils.drawVelocityVectors(
                    incons_img, next_position, relative_disp=False, color=(0, 0, 255)
                )
                plt.imsave(
                    os.path.join(base_fn, DRAW_DIR, img_num + "_draw.png"),
                    incons_img.astype("uint8"),
                )

            if self.create_velocity:
                image = velocity.copy()
                plt.imsave(
                    os.path.join(base_fn, VL_DIR, img_num + "_velocity.png"),
                    image.astype("uint8"),
                )
            ...
            y = velocity[:, :, 1]
            z = velocity[:, :, 2]
            speed_superpixel = utils.vectorDistance(x, y, z)
            # mask_edges = mark_boundaries(speed_superpixel.astype(np.int), speed_mask, color=(1,0,0))

            np.save(
                os.path.join(base_fn, NP_DIR, img_num + "_superpixel.npy"),
                speed_superpixel,
            )
            ...
            plt.savefig(
                os.path.join(
                    base_fn, SUPER_PIXEL_DIR, "{0}_superpixel.png".format(img_num)
                ),
                bbox_inches="tight",
                dpi=150,
            )
            ...
            # Calculate the velocity and the orientation
            velocity = utils.calculateVelocityAndOrientationVectorsVectorised(
